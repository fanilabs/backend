...
| `GET` | `/api/v1/notifications` | List the authenticated user's own notifications, newest first — optional `status` (`PENDING`/`SENT`/`FAILED`), `limit` (default 20, max 100), and `before` (ISO timestamp cursor) query params |
