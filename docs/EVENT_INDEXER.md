...
`notifications` is the sixth subscriber (`src/modules/notifications/infrastructure/event-subscription.ts`/`dispatch-notifications-from-event.ts`), and the first one that isn't syncing a read model — it turns a handful of events into `Notification` rows instead. Deliberately narrow scope: only events where a directly-available actor address is both present *and* someone worth notifying are handled (`delivery.driver_assigned`, `escrow.delivery_disputed`, `escrow.escrow_released` — payload `(driver, payout, fee)`, verified against this doc's own escrow section below — `dispute-resolution.dispute_raised`, all four `identity-reputation` events, and five of `fleet`'s events). Excluded for three different reasons, not one: `delivery_confirmed`/`delivery_cancelled`/`DeliveryInTransit`/`escrow_refunded`/the escrow-layer `dispute_resolved` genuinely carry no address at all; `dispute_resolved_refund`/`_split`/`_payout` carry only the resolving *admin's* own address (not useful to notify); and `delivery_created` carries the sender's address (`payload[1]`) but notifying someone of their own just-submitted action adds nothing. `escrow_funded`'s payload contents beyond "no driver/token" aren't documented anywhere verifiable, so it's treated the same as "no address" rather than guessed at. An address with no linked local account is silently skipped, not an error. See `dispatch-notifications-from-event.ts`'s header comment for the full per-event breakdown.
